    ...
    <nav className="w-full bg-white border-b border-gray-100 px-6 py-4 flex justify-between items-center sticky top-0 z-50">
      ...
      <div className="flex items-center gap-4">
        {!loading && (
          user ? (
            <>
              <Link href="/dashboard" className="text-sm font-medium text-gray-700 hover:text-brand transition-colors">
                Dashboard
              </Link>
              <button
            ...
            <Link href="/auth/login" className="px-4 py-2 bg-brand hover:bg-[#66b300] text-white text-sm font-medium rounded-lg transition-colors">
              Sign In
            </Link>
          )
        )}

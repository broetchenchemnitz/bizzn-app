...
export default async function ProjectWorkspacePage({
  ...
  return (
    /*
     * iOS / WebKit sticky fix — DOM structure rules:
     * ─────────────────────────────────────────────
     * 1. <header> MUST be a sibling of overflow wrapper, NEVER inside it.
     * 2. No ancestor of <header> up to <body> may have overflow != visible.
     * 3. z-index: 50 (header) < z-index: 50 (global Navbar) — same layer is
     *    fine because Navbar is a separate stacking context (sticky in flex parent).
     * 4. backdrop-filter requires -webkit- prefix for Safari ≤ 15.
     * 5. Grid body margin-top: pt-4 prevents margin-collapse after
     *    header extraction from flow.
     *
     * Verified ancestor chain (no overflow):
     *   <body> → <main> → <div#root>
     *     ├── <header sticky z-50>   ← CLEAN ANCESTOR CHAIN
     *     └── <div overflow-x-hidden> ← contains NO sticky children
     */
    <div className="min-h-screen bg-[#1A1A1A]">

      {/* ══════════════════════════════════════════════════════════
          STICKY PROJECT BREADCRUMB
          - z-50: sits above all content layers (same z as Navbar;
            separate contexts means they don't conflict)
          - bg-[#1A1A1A]/95 + backdrop-blur: solid base with subtle
            translucency — prevents content bleed-through on scroll
          - -webkit-backdrop-filter: required for Safari < 15
          ══════════════════════════════════════════════════════════ */}
      <header
        className="sticky top-[70px] z-50 border-b border-white/5 px-4 sm:px-8 py-3.5 flex items-center gap-3 min-w-0 backdrop-blur-md"
        style={{
      ...
      {/* ══════════════════════════════════════════════════════════
          GRID BODY — overflow-x-hidden is LOCAL to this wrapper.
          The <header> above is a DOM sibling, so its sticky
          positioning is unaffected by this overflow declaration.
          pt-4: prevents CSS margin-collapse between the header's
          bottom border and the first grid card's margin-top.
          ══════════════════════════════════════════════════════════ */}

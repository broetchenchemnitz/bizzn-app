...
- **Echtzeit-Auszahlungen aktiviert:** Die nächtliche Cashflow-Engine führt ab sofort echte Stripe-Transfers (`stripe.transfers.create`) aus. Sicherheitsmechanismus: `payout_status`-Spalte verhindert Doppelabbuchungen — jede Bestellung wird exakt einmal ausgezahlt. Migration 006 enthält CHECK-Constraint und Composite-Index für effiziente Cron-Abfragen.
- **Self-Service Web-Adresse:** Restaurant-Partner können ihre individuelle Bizzn-Subdomain jetzt eigenständig im Dashboard konfigurieren (z. B. `mein-restaurant.bizzn.de`). Validierung im Frontend (Echtzeit-Bereinigung) und im Backend (Format, Länge, Einzigartigkeit). Bei Doppelungen erscheint eine verständliche Fehlermeldung.
- **KDS Auszahlungs-Transparenz:** Das Kitchen Display System zeigt jetzt eine vierte Spalte „Ausgeliefert“ mit allen heutigen abgeschlossenen Bestellungen. Jede Karte trägt ein Echtzeit-Badge: 🟢 „Ausgezahlt“ (sobald der nächtliche Cron den Stripe-Transfer ausgeführt hat) oder ⏳ „Ausstehend“. Die Live-Leiste zeigt zusätzlich eine kompakte Zusammenfassung aller ausgezahlten und ausstehenden Beträge.
- **Radikale Dashboard-Neugestaltung:** Das Restaurant-Dashboard wurde von einem linearen Block-Layout in ein professionelles 12-Spalten-Grid umgebaut. Der Hauptbereich (links, 8 Spalten) zeigt KPI-Karten und Schnell-Aktionen. Die Seitenleiste (rechts, 4 Spalten) enthält die Web-Adress-Eingabe und den Bestellkanal-Status. Eine neue Navigationsleiste am oberen Rand zeigt Projektname und Echtzeit-Indikator dauerhaft an — ohne redundante Status-Balken.

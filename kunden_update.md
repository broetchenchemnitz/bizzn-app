...
- **Echtzeit-Bestelltracking:** Kunden werden nach dem Bestellen automatisch auf `[slug].bizzn.de/order/[id]` weitergeleitet. Die Seite zeigt Status, Fortschrittsanzeige und Bestelldetails — live aktualisiert via Supabase Realtime, sobald die Küche den Status ändert.
- **In-Store QR-Code-Modus:** Gäste können per QR-Code am Tisch bestellen (`?table=3&mode=kiosk`). Die Speisekarte sperrt sich automatisch auf „Vor Ort“, zeigt die Tischnummer an und speichert diese mit der Bestellung. Tischnummern werden in der Datenbank hinterlegt (Migration 005).
